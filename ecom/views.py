...
from ecommerce.credentials import MpesaAccessToken, LipanaMpesaPpassword
...
def token(request):
    consumer_key = 'v1kPFlj2iuxDpeBP7EJ2aD2qushYsMUJHGXdBzhVdPqvvYiQ'
    consumer_secret = 'cJlol8jez8DmYqikYljAyASTqEyFr4dNTi2F0HyPsgoDVW8fLKFObh3GJ5rQDfsP'
...
def stk(request):
    if request.method =="POST":
        ...
        # Check for success based on ResponseCode
        if response_data.get("ResponseCode") == "0":
            # Redirect to payment success page
            return redirect("payment-success")  # Use the name of your success template
        else:
            # Handle failure by returning an error message
            return HttpResponse(f"Payment failed: {response_data.get('errorMessage', 'Unknown error')}")
